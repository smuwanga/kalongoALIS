		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<div class="form-group">
					...
					<!-- <h1 class="panel-title" style="text-align:left"><strong>PATIENT DETAILS</strong></h1> -->
				...
				<div class="form-group">
					...
					{{ Form::label('personnel_age', 'Age', array('class' =>'col-sm-2 required ')) }}
				...
				<div class="form-group">
					...
					{{ Form::text('personnel_age', Input::old('personnel_age'), array('class' => 'form-control col-sm-4', 'placeholder' => 'If Status is not known')) }}
	...
	<div class="panel panel-primary">
		...
		<div class="panel-body">
			...
			<div class="form-group">
				...
				{{ Form::label('admimission_date', 'Sample Collection Date:', array('class' =>'col-sm-2 ')) }}
				{{ Form::text('admimission_date', Input::old('admimission_date'), array('class' => 'form-control standard-datepicker col-sm-4', 'placeholder' => 'DD/ MM /YYYY')) }}
			</div>
			...
			<span>For known HIV Exposed infants, information to enter on this Request Form should be picked from EI register. <br> For all other infants,
				use the inpatient register (HMIS 054)</span></div> <br> <span><strong>Note: R1 =</strong> Any repeat before 2nd PCR,  <strong>R2 = </strong> Any repeat after 2nd PCR before 18 months
				...
				<span>If Mother is HIV positive, Mother's PMTCT ARV's (Select & check circle)</span>
					...
					<div class="form-group">
						{{ Form::label('personnel', 'Circle Number:', array('class' =>'col-sm-2')) }}
						<div class="radio-inline">{{ Form::radio('personnel_gender', 'Lifelong ART', false) }} <span class="input-tag">Lifelong ART</span></div>
						<div class="radio-inline">{{ Form::radio("personnel_gender", 'No ART', false) }} <span class="input-tag">No ART</span></div>
						<div class="radio-inline">{{ Form::radio("personnel_gender", 'Unknown', false) }} <span class="input-tag">UNKNOWN</span></div>
					</div>
				...
				<div class="radio-inline">{{ Form::radio('personnel_gender', 'Lifelong ART', false) }} <span class="input-tag">Daily NVP from birth to 6 weeks</span></div>
				<div class="radio-inline">{{ Form::radio("personnel_gender", 'No ART', false) }} <span class="input-tag">NVP for 12 weeks for high risk infants</span></div>
				<div class="radio-inline">{{ Form::radio("personnel_gender", 'Unknown', false) }} <span class="input-tag">No ARVs taken at birth</span></div>
				<div class="radio-inline">{{ Form::radio("personnel_gender", 'Unknown', false) }} <span class="input-tag">UNKNOWN</span></div>
					<br>
					<br>
				</div>
